            ...
            { auth.token && Object.entries(faves).length > 0 ?
                <div className="favourites__container">
                    {Object.entries(faves).map((item) =>
                        (
                            <div onClick={() => {
                                detailedViewHandler(item[1]._id)
                            }} className="favourites__item" key={item[1]._id}>
                                <div className="favourites__image-wrap">
                                    <img className="favourites__image" src={item[1].img}/>
                                    <figure className="favourites__icon-heart" onClick={(e) => {
                                        favesHandler(e,item[1]._id)}}>
